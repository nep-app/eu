    ...
    if (isSlot2 && isSealed && !isRevealed) return (
      <div style={{ ...CARD, borderTop:`2px solid ${accent}` }}>
        ...
        <div style={{ textAlign:"center", padding:"24px 0" }}>
          ...
          {capData.sealedAt && <div style={{ fontSize:10, color:"#334155", marginTop:8 }}>Selada a {capData.sealedAt}</div>}
        </div>
      </div>
    );

    // cap1 sealed (user can read it back)
    if (!isSlot2 && isSealed) return (
      <div style={{ ...CARD, borderTop:`2px solid ${accent}` }}>
        ...
        <div style={{ textAlign:"center", padding:"20px 0" }}>
          ...
          {capData.sealedAt && <div style={{ fontSize:10, color:"#475569", marginTop:6 }}>Selada a {capData.sealedAt}</div>}
        </div>
      </div>
    );

    // cap2 revealed by admin
    if (isSlot2 && isRevealed) return (
      <div style={{ ...CARD, borderTop:`2px solid ${accent}` }}>
        <div style={{ fontSize:24, marginBottom:6 }}>💌</div>
        <div style={{ fontWeight:900, color:accent, marginBottom:12 }}>A TUA CÁPSULA FINAL</div>
        {capData.text && <div style={{ fontSize:14, color:"#f1f5f9", lineHeight:1.7, whiteSpace:"pre-wrap", marginBottom:12 }}>{capData.text}</div>}
      ...
      <div style={{ ...CARD, borderTop:`2px solid ${accent}` }}>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:16 }}>
          {[["texto","✏️ Texto"],["foto","📸 Foto"],["audio","🎙 Áudio"],["video","🎬 Vídeo"]].map(([t,l]) => (
            <button key={t} onClick={() => setMediaTab(t)} style={{
              ...
              background: mediaTab === t ? `${accent}18` : "rgba(255,255,255,0.04)",
              color: mediaTab === t ? accent : TXT_MUT,
        ...
        {mediaTab === "texto" && (
          <>
            ...
            <textarea value={capData.text || ""} onChange={e => saveText(e.target.value)}
              style={{ ...INP, fontSize:13, resize:"none", marginBottom:12 }} rows={5}
        ...
        {mediaTab === "audio" && (
          <div style={{ marginBottom:16, textAlign:"center" }}>
            ...
            {uploading && <div style={{ fontSize:12, color:"#64748b", marginTop:10 }}>A guardar...</div>}
          </div>
        )}
