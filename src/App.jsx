...
export default function App() {
  ...
  // ── MURAL ────────────────────────────────────────────────────────
  var [channel,  setChannel]  = useState("csi");
  var [posts,    setPosts]    = useState({ csi:[], monitor:[], olx:[], orienta:[], backstage:[], coffee:[] });
  ...
  var [adminMissionXp,  setAdminMissionXp]  = useState(10);
  var [xpGain,          setXpGain]          = useState(null);
  ...
  useEffect(function() {
    ...
    var unsub = onSnapshot(doc(db, "config", "weeklyLeaderboard"), function(snap) {
      if (snap.exists()) {
        var d = snap.data();
        setLeaderboard(d.week === getWeekKey() ? (d.scores || {}) : {});
      } else {
        setLeaderboard({});
      }
    });
  ...
  useEffect(function() {
    ...
    var unsub = onSnapshot(collection(db, "missions"), function(snap) {
      setMissions(snap.docs.map(function(d) { return Object.assign({ id:d.id }, d.data()); }));
    });
    return unsub;
  }, [user]);
  ...
  async function addXp(amount) {
    ...
    try { await updateDoc(lbRef, lbUpdate); }
    catch(e) {
      var init = { week: wk, scores: {} };
      init.scores[user.username] = { xp: nv, name: user.realName, color: user.color };
      await setDoc(lbRef, init);
    }
  ...
  async function completeMission(missionId) {
    ...
    var newComp = completedMissions.concat([missionId]);
  ...
  async function addAdminMission() {
    ...
    await addDoc(collection(db, "missions"), {
      text: adminMissionTxt, xp: adminMissionXp, week: getWeekKey(), createdAt: Date.now()
    });
    setAdminMissionTxt("");
  }
  async function deleteAdminMission(id) {
    if (!window.confirm("Apagar esta missão?")) return;
  ...
  async function reactPost(pid, reaction) {
    var cur = (posts[channel]||[]).find(function(p) { return p.id === pid; });
    if (!cur) return;
    var reactions = Object.assign({ heart:0, fire:0, clap:0, think:0 }, cur.reactions || (cur.likes ? { heart:cur.likes, fire:0, clap:0, think:0 } : {}));
    var reactedBy = Object.assign({ heart:[], fire:[], clap:[], think:[] }, cur.reactedBy || {});
    var who = reactedBy[reaction] || [];
    if (who.includes(user.username)) {
      reactions[reaction] = Math.max(0, (reactions[reaction]||0) - 1);
      reactedBy[reaction] = who.filter(function(u){ return u !== user.username; });
    } else {
      reactions[reaction] = (reactions[reaction]||0) + 1;
      reactedBy[reaction] = who.concat([user.username]);
    }
    await updateDoc(doc(db, "forum", channel, "posts", pid), { reactions:reactions, reactedBy:reactedBy });
  }
...
async function submitAnswer() {
  ...
  var ADMIN_TABS = [["geral","📊 Geral"],["mural","🌐 Fórum"],["partilhas","📂 Partilhas"],["tasks","✅ Tarefas"],["agenda","📅 Agenda"],["missoes","🎯 Missões"],["msgs","💬 Msgs"],["users","👥 Utilizadores"]];
    return (
                    ...
                    <div style={{ display:"flex", gap:10 }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ marginTop:9, display:"flex", gap:12, alignItems:"center" }}>
                          {FORUM_REACTIONS.map(function(r) {
                            var rcts = p.reactions || (r.id==="heart" ? { heart:p.likes||0 } : {});
                            var cnt = rcts[r.id] || 0;
                            return (<span key={r.id} onClick={function(){reactPost(p.id,r.id);}} style={{ fontSize:12, color:"#94a3b8", cursor:"pointer", marginRight:6 }}>{r.icon}{cnt>0?" "+cnt:""}</span>);
                          })}
...
{[["texto","✏️ Texto"],["mood","🌡️ Mood"],["3p","💡 3 Palav."],["completar","🔤 Completar"],["semana","⭐ Avaliação"], ["foto", "📸 Foto"], ["video", "🎥 Vídeo"], ["audio", "🎙️ Áudio"]].map(function(opt) {                    var isSel = activeQModeEdit.includes(opt[0]);
                ...
                {JEEP_LIST.map(function(j) {
                  ...
                  return (
                    <div key={j.username} style={{ padding:"10px 0", borderBottom:"1px solid #f1f5f9" }}>
                      ...
                      {d.answered && (
                        <div style={{ marginLeft:16, marginTop:8, fontSize:12, color:"#374151", background:"#f8fafc", borderRadius:8, padding:"10px", border:"1px solid #e2e8f0" }}>
                          {/* Se for FOTO */}
                          {atype === "foto" && d.answerMedia ? (
                            <div style={{ textAlign: "center" }}>
                              <img src={d.answerMedia} alt="Resposta" style={{ maxWidth: "100%", borderRadius: 8, marginTop: 5, boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }} />
                              <a href={d.answerMedia} target="_blank" rel="noreferrer" style={{ display: "block", fontSize: 11, color: "#7C3AED", marginTop: 5, fontWeight: 700 }}>Ver foto original ↗</a>
                            </div>
                          ) : 
                          /* Se for VÍDEO */
                          atype === "video" && d.answerMedia ? (
                            <div style={{ textAlign: "center" }}>
                              <video controls style={{ width: "100%", borderRadius: 8, marginTop: 5 }}>
                                <source src={d.answerMedia} />
                                O teu navegador não suporta vídeos.
                              </video>
                              <a href={d.answerMedia} target="_blank" rel="noreferrer" style={{ display: "block", fontSize: 11, color: "#7C3AED", marginTop: 5, fontWeight: 700 }}>Ver vídeo original ↗</a>
                            </div>
                          ) : 
                          /* Se for ÁUDIO */
          ...
          {adminTab === "missoes" && (
            <div>
              <div style={CARD}>
                <div style={SL}>Nova Missão da Semana</div>
                <div style={{ fontSize:12, color:"#64748b", marginBottom:12 }}>Qualquer missão que adicionas esta semana faz parte das missões semanais. Quando os participantes as completarem ganham XP.</div>
                <input value={adminMissionTxt} onChange={function(e){setAdminMissionTxt(e.target.value);}} placeholder="Descrição da missão (ex: Preenche o PIA esta semana)..." style={{ width:"100%", padding:"11px 14px", borderRadius:12, border:"2px solid #e8edf2", fontSize:13, outline:"none", boxSizing:"border-box", marginBottom:8 }}/>
                <div style={{ display:"flex", gap:8, marginBottom:12, alignItems:"center" }}>
                  <div style={{ fontSize:12, color:"#64748b" }}>XP a ganhar:</div>
                  {[5,10,15,20].map(function(v){return(
                    <button key={v} onClick={function(){setAdminMissionXp(v);}} style={{ padding:"6px 14px", borderRadius:20, border:adminMissionXp===v?"2px solid #22c55e":"2px solid #e8edf2", background:adminMissionXp===v?"#22c55e15":"white", fontSize:12, fontWeight:700, cursor:"pointer", color:adminMissionXp===v?"#16a34a":"#64748b" }}>+{v}</button>
                  );})}
                </div>
                <button onClick={addAdminMission} style={{ background:"#1e293b", color:"white", border:"none", borderRadius:12, padding:"11px 18px", fontSize:13, fontWeight:700, cursor:"pointer" }}>Adicionar Missão</button>
              </div>
              <div style={CARD}>
                <div style={SL}>Missões Ativas — {getWeekKey()}</div>
                {missions.filter(function(m){ return m.week === getWeekKey(); }).length === 0 ? (
                  <div style={{ textAlign:"center", padding:"20px 0", color:"#94a3b8", fontSize:13 }}>Ainda não há missões esta semana.</div>
                ) : (
                  missions.filter(function(m){ return m.week === getWeekKey(); }).map(function(m) {
                    var doneCount = JEEP_LIST.filter(function(j){
                      var d = allShared[j.username] || {};
                      return (d.completedMissions||[]).includes(m.id);
                    }).length;
                    return (
                      <div key={m.id} style={{ padding:"12px 14px", background:"#f8fafc", borderRadius:14, marginBottom:8, border:"1px solid #e8edf2", display:"flex", alignItems:"center", gap:10 }}>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:13, fontWeight:700 }}>{m.text}</div>
                          <div style={{ fontSize:11, color:"#94a3b8", marginTop:3 }}>+{m.xp} XP · {doneCount}/{JEEP_LIST.length} completaram</div>
                          <div style={{ background:"#e8edf2", borderRadius:99, height:4, marginTop:6, overflow:"hidden" }}><div style={{ background:"#22c55e", height:"100%", width:(doneCount/JEEP_LIST.length*100)+"%", borderRadius:99 }}/></div>
                        </div>
                        <button onClick={function(){deleteAdminMission(m.id);}} style={{ background:"none", border:"none", color:"#ef4444", cursor:"pointer", fontSize:16 }}>✕</button>
                      </div>
                    );
                  })
                )}
              ...
              {missions.filter(function(m){ return m.week !== getWeekKey(); }).length > 0 && (
                <div style={CARD}>
                  <div style={SL}>Missões Anteriores</div>
                  {missions.filter(function(m){ return m.week !== getWeekKey(); }).slice(-5).reverse().map(function(m) {
                    return (
                      <div key={m.id} style={{ padding:"10px 0", borderBottom:"1px solid #f1f5f9", display:"flex", alignItems:"center", gap:8 }}>
                        <div style={{ flex:1 }}><div style={{ fontSize:12, color:"#94a3b8", textDecoration:"line-through" }}>{m.text}</div><div style={{ fontSize:10, color:"#cbd5e1" }}>{m.week} · +{m.xp} XP</div></div>
                        <button onClick={function(){deleteAdminMission(m.id);}} style={{ background:"none", border:"none", color:"#ef4444", cursor:"pointer", fontSize:14 }}>✕</button>
                      </div>
                    );
                  })}
                </div>
...
<div style={CARD}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG, fontFamily:"system-ui,sans-serif", maxWidth:420, margin:"0 auto", display:"flex", flexDirection:"column" }}>
      ...
      {xpGain && (
        <div style={{ position:"fixed", top:70, left:"50%", transform:"translateX(-50%)", zIndex:999, background:"linear-gradient(135deg,#22c55e,#16a34a)", color:"white", padding:"10px 22px", borderRadius:30, fontSize:15, fontWeight:800, boxShadow:"0 4px 20px rgba(34,197,94,0.45)", pointerEvents:"none", whiteSpace:"nowrap" }}>{xpGain}</div>
      )}
          ...
          <div style={{ padding:"18px 16px" }}>
            ...
            {(function(){
              ...
              return (
                <div style={CARD}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                    <div style={SL}>⚡ Tabela Semanal</div>
                    <div style={{ fontSize:11, color:"#94a3b8" }}>reinicia toda semana</div>
                  </div>
                  {top3.length === 0 ? (
                  ...
                  <div style={{ marginTop:10, padding:"8px 12px", background:inTop3?C+"10":"#f8fafc", borderRadius:10, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            {(function(){
              var weekMissions = missions.filter(function(m){ return m.week === getWeekKey(); });
              if (weekMissions.length === 0) return null;
              var doneCnt = weekMissions.filter(function(m){ return completedMissions.includes(m.id); }).length;
              var allDone = doneCnt === weekMissions.length;
              return (
                <div style={Object.assign({},CARD,{border:allDone?"2px solid #22c55e":"1.5px solid "+C+"25"})}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                    <div style={SL}>🎯 Missões desta Semana</div>
                    <div style={{ fontSize:11, fontWeight:800, color:allDone?"#16a34a":C }}>{doneCnt}/{weekMissions.length}</div>
                  </div>
                  {allDone && (<div style={{ fontSize:13, fontWeight:700, color:"#16a34a", marginBottom:10, padding:"8px 12px", background:"#f0fdf4", borderRadius:10 }}>🎉 Completaste todas as missões da semana!</div>)}
                  {weekMissions.map(function(m) {
                    var done = completedMissions.includes(m.id);
                    return (
                      <div key={m.id} onClick={function(){ if(!done) completeMission(m.id); }} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:12, background:done?"#f0fdf4":"#f8fafc", marginBottom:6, border:done?"1.5px solid #bbf7d0":"1.5px solid #e8edf2", cursor:done?"default":"pointer" }}>
                        <div style={{ width:22, height:22, borderRadius:6, background:done?"#22c55e":"white", border:done?"none":"2px solid #e8edf2", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{done&&<span style={{ color:"white", fontSize:13, fontWeight:900 }}>✓</span>}</div>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:13, fontWeight:done?400:600, color:done?"#94a3b8":"#0f172a", textDecoration:done?"line-through":"none" }}>{m.text}</div>
                        </div>
                        <div style={{ fontSize:11, fontWeight:700, color:"#22c55e" }}>+{m.xp} XP</div>
                      </div>
                    );
                  })}
                </div>
                  ...
                  <div style={{ display:"flex", gap:10 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ marginTop:9, display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                        {FORUM_REACTIONS.map(function(r) {
                          var reactions = p.reactions || (r.id==="heart" ? { heart:p.likes||0 } : {});
                          var cnt = reactions[r.id] || 0;
                          var myReact = ((p.reactedBy||{})[r.id]||[]).includes(user.username);
                          return (
                            <span key={r.id} onClick={function(){reactPost(p.id,r.id);}} style={{ fontSize:12, cursor:"pointer", padding:"3px 8px", borderRadius:20, background:myReact?C+"18":"#f1f5f9", color:myReact?C:"#64748b", fontWeight:myReact?700:400 }}>{r.icon}{cnt>0?" "+cnt:""}</span>
                          );
                        })}
                        <span onClick={function(){setReplyTo(replyTo===p.id?null:p.id);setExpanded(p.id);}} style={{ fontSize:12, color:"#94a3b8", cursor:"pointer", fontWeight:600 }}>💬 Responder</span>
